<div class="row justify-content-center">
    <div class="col-12">
        <div class="card">
            ...
            @else
                <h5 class="card-header border-bottom text-uppercase bg-primary bg-soft text-primary">All Comments By: {{auth()->user()->name}}</h5>
            @endif
            ...
            <div class="card-body">
                ...
                <div class="card">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        ...
                        @else
                            <h6 class="mb-0">All Comments By: {{auth()->user()->name}}</h6>
                        @endif
                    </div>
                    <div class="card-body">
                        <!--Filters-->
                        ...
                        <div class="table-responsive">
                            <table class="table mb-0">
                                ...
                                <tbody>
                                @if(auth()->user()->isAdmin() && $allComments->count() > 0)
                                    @foreach($allComments as $index => $comment)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{route('dashboard.comments.edit', $comment->id)}}" class="btn btn-info waves-effect">Edit</a>
                                                <a href="#" class="btn btn-danger waves-effect" wire:click.prevent="deleteComment({{$comment->id}})">Delete</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                @elseif(!auth()->user()->isAdmin() && $writerRoleComments->count() > 0)
                                    @foreach($writerRoleComments as $index => $comment)
                                        <tr>
                                            <th scope="row">{{$loop->iteration}}</th>
